...
{
	public:
		CRollerCoaster(uint32_t, uint32_t);
		virtual ~CRollerCoaster();

		void AppInit();
		void AppExit();
		void AppCycle(int32_t aFrame);

		void FlatShading();
		void SmoothShading();
		void SetPerspectiveCorrection(bool aPerspective);
		bool GetPerspectiveCorrection();
		void SetMipmapping(bool aMipmapping);
		bool GetMipmapping();
