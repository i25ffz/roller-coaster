...
void rotate(point *r, point *v, float t, point *p){
	r->x = ( v->x * v->x + (float)cos(t) * (1.0f - (v->x * v->x)) ) * p->x +
	       ( v->x * v->y * (1.0f - (float)cos(t)) - v->z * (float)sin(t) ) * p->y +
	       ( v->z * v->x * (1.0f - (float)cos(t)) + v->y * (float)sin(t) ) * p->z;
	r->y = ( v->x * v->y * (1.0f - (float)cos(t)) + v->z * (float)sin(t) ) * p->x +
	       ( v->y * v->y + (float)cos(t) * (1.0f - (v->y * v->y)) ) * p->y +
	       ( v->y * v->z * (1.0f - (float)cos(t)) - v->x * (float)sin(t) ) * p->z;
	r->z = ( v->z * v->x * (1.0f - (float)cos(t)) - v->y * (float)sin(t) ) * p->x +
	       ( v->y * v->z * (1.0f - (float)cos(t)) + v->x * (float)sin(t) ) * p->y +
	       ( v->z * v->z + (float)cos(t) * (1.0f - (v->z * v->z)) ) * p->z;
}
